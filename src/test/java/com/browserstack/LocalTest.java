...
public class LocalTest extends BrowserStackTestNGTest {
    ...
    AndroidElement searchElement = (AndroidElement) new WebDriverWait(dr, 30).until(
        ExpectedConditions.elementToBeClickable(MobileBy.id("com.example.android.basicnetworking:id/test_action")));
    searchElement.click();
    AndroidElement insertTextElement = (AndroidElement) new WebDriverWait(dr, 30).until(
        ExpectedConditions.elementToBeClickable(MobileBy.className("android.widget.TextView")));

    AndroidElement testElement = null;
    List<AndroidElement> allTextViewElements = dr.findElementsByClassName("android.widget.TextView");
    Thread.sleep(10);
